...
class Visualizer:
    ...
    def _plot_all_cells_summary(self, analysis: FileData, fit_model: str):
        ...
        for idx, cell_id in enumerate(all_cell_ids):
            ...
            if not np.isnan(red_intensity) and not np.isnan(green_intensity):
                ...
                # 构建下方标签：R、G、Area文字 + 红绿正方形色块，整行居中排列
                info_text = f'R:{red_intensity:.0f}  G:{green_intensity:.0f}  Area:{cell_size}'
                ax.text(0.38, -0.24, info_text, transform=ax.transAxes, fontsize=8,
                       ha='right', va='center')
                
                # 红色正方形 - 颜色深浅表示强度（居中排列）
                red_color = (1.0, 1.0 - red_norm * 0.8, 1.0 - red_norm * 0.8)  # 从浅粉到深红
                rect_red = plt.Rectangle((0.40, -0.27), 0.06, 0.06, transform=ax.transAxes,
                ...
                rect_green = plt.Rectangle((0.48, -0.27), 0.06, 0.06, transform=ax.transAxes,
    ...
    def generate_ratio_vs_t50_analysis(self, analyses: List[FileData], reaction_df, 
        ...
        # 统计分析和作图已迁移到 statistics_analysis.py 模块
